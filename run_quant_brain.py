...
def get_stock_report(ticker):
    ...
    latest = df.iloc[-1]
    return {
        "ticker": ticker, "close": float(latest['Close']), "ma20": float(latest['MA20']),
        "upper": float(latest['Upper_Band']), "lower": float(latest['Lower_Band']), "rsi": float(latest['RSI'])
    }
...
def main():
    ...
    prompt = f"""
    你是一位精通技術分析的頂級量化對沖基金經理。
    請根據以下計算出的真實技術指標數據，為投資人撰寫一份精煉、美觀的手機盤前診斷報告。
    
    【重要排版規範】：
    1. 請完全使用繁體中文回答。
    2. 請務必、嚴格使用以下 HTML 標籤包裹對應內容（不要自作聰明漏掉）：
       - 股票代碼兩旁必須加上 <b> 標籤，例如：<b>NVDA</b>、<b>TSLA</b>。
       - 所有價格與數值兩旁必須加上 <code> 標籤，例如：<code>$211.14</code>、<code>84.3</code>。
    ...
    5. 每一檔個股的分析請嚴格依照以下範例格式輸出，不要有多餘的廢話：
    
    📌 <b>[股票代碼]</b> (收盤價: <code>[收盤價]</code>)
    • 20日均線: <code>[MA20]</code> | RSI: <code>[RSI]</code>
    • 策略短評: [一兩句話銳評，若觸發超買超賣，請在此加上 <b>【短線超買過熱】</b> 或 <b>【短線超跌打底】</b> 的粗體警示。]
    
    【分析指南】：
    - 如果收盤價接近或突破上軌，且 RSI > 70，提示「短線超買過熱，注意回檔風險」。
    - 如果收盤價接近或跌破下軌，且 RSI < 30，提示「短線超跌打底，關注反彈契機」。
